...
// VIGTIGT: Vi henter 'useUser' for at kunne opdatere hele appen
...
export default function SecureLayoutClient({
  children,
  user: initialUser, // Omdøbt for ikke at konflikte med context
  dict,
  lang,
  initialPathname,
  ...
  // Hent den globale bruger fra Context
  ...
  // Tjek om det er en rigtig dev
  const isDtlEmployee = initialUser.role === UserRole.Developer || initialUser.role === UserRole.Tester;

  // --- ADGANGSKONTROL LOGIK (Bruger nu 'user' fra context) ---
  const checkAccess = (role: UserRole, moduleKey: string) => {
    ...
    switch (moduleKey) {
      ...
      case 'club': 
        return [UserRole.ClubOwner, UserRole.ClubAdmin, UserRole.Management, UserRole.AcademyDirector, UserRole.TeamLead].includes(role);
      case 'players': 
        return [UserRole.HeadOfCoach, UserRole.Coach, UserRole.AssistantCoach, UserRole.Scout, UserRole.AcademyDirector, UserRole.ClubAdmin, UserRole.YouthDevelopmentCoach].includes(role);
      case 'video':
        return [UserRole.HeadOfCoach, UserRole.Coach, UserRole.AssistantCoach, UserRole.Analyst, UserRole.Scout, UserRole.Player, UserRole.TransitionsCoach, UserRole.KeeperCoach].includes(role);
      case 'scouting':
        return [UserRole.Scout, UserRole.HeadOfCoach, UserRole.AcademyDirector, UserRole.ClubOwner, UserRole.ClubAdmin].includes(role);
      case 'chat': return true;
      default: return false;
    }
  };

  const accessibleModules = modules.filter(m => checkAccess(user.role, m.nameKey));

  // --- OPDATERET SIMULERING ---
  const handleLevelChange = (level: SubscriptionLevel) => {
    // Opdaterer HELE appen
    setUser({ ...user, subscriptionLevel: level });
  };
  ...
  const roleGroups = [
    { title: 'Ledelse & Admin', roles: [UserRole.ClubOwner, UserRole.ClubAdmin, UserRole.Management, UserRole.AcademyDirector] },
    { title: 'Trænerstab (General)', roles: [UserRole.HeadOfCoach, UserRole.YouthDevelopmentCoach, UserRole.Coach, UserRole.AssistantCoach] },
    { title: 'Specialister', roles: [UserRole.KeeperCoach, UserRole.TransitionsCoach, UserRole.IndividualCoach, UserRole.DefenseCoach, UserRole.AttackCoach, UserRole.DeadBallCoach] },
    { title: 'Performance & Sundhed', roles: [UserRole.FitnessCoach, UserRole.Physio] },
    { title: 'Analyse & Scouting', roles: [UserRole.Analyst, UserRole.Scout] },
    { title: 'Spillere & Hold', roles: [UserRole.Player, UserRole.Parent, UserRole.TeamLead, UserRole.ExternalPlayer] },
    { title: 'Teknisk', roles: [UserRole.Developer, UserRole.Tester] }
  ];

  const Sidebar = () => (
    <aside className={`${isSidebarOpen ? 'w-64 lg:w-52 2xl:w-64' : 'w-20'} bg-white border-r flex flex-col shrink-0 transition-all duration-300 lg:block ${isSidebarOpen ? 'max-lg:fixed max-lg:inset-y-0 max-lg:left-0 max-lg:z-30 max-lg:pt-12' : 'max-lg:hidden'}`}>
      <nav className="flex-1 mt-2 space-y-1 p-2 overflow-y-auto custom-scrollbar">
        {accessibleModules.map((module) => {
          const sidebarDict = dict.sidebar || {};
          const isParentActive = module.path ? cleanCurrentRoute.startsWith(module.path) : module.subModules?.some(sub => cleanCurrentRoute.startsWith(sub.path));
  ...
  const Header = () => {
    ...
    const handleViewChange = (view: 'grid' | 'canvas') => {
        ...
        if (view === 'grid') { currentParams.delete('view'); } else { currentParams.set('view', 'canvas'); }
      ...
      <div className="w-full bg-white flex-shrink-0 z-20 relative border-b">
        <div className="h-12 flex items-center justify-between px-2 sm:px-4">
          <div className="flex items-center">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-1 rounded-full hover:bg-gray-100 cursor-pointer mr-2 sm:mr-3">
          ...
          {isDashboardRoute && (
            <div className="flex items-end gap-0.5 mr-auto ml-20"> 
              <button onClick={() => handleViewChange('grid')} className={`flex items-center gap-1 px-3 py-0.5 rounded-full text-xs font-semibold transition-colors border border-transparent ${currentView === 'grid' ? 'text-orange-600' : 'text-gray-600 hover:bg-gray-100'}`}><LayoutGrid className="w-4 h-4" /> Grid</button>
              <button onClick={() => canUseCanvas && handleViewChange('canvas')} disabled={!canUseCanvas} title={!canUseCanvas ? (lang === 'da' ? 'Canvas kræver Elite eller Enterprise adgang' : 'Canvas requires Elite or Enterprise access') : ''} className={`flex items-center gap-1 px-1 py-0.5 rounded-full text-xs font-semibold transition-colors border border-transparent ${currentView === 'canvas' && canUseCanvas ? 'text-orange-600' : 'text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed'}`}><Layers className="w-4 h-4" /> Canvas</button>
            </div>
          )}
          <div className="relative" ref={profileRef}>
            ...
            {isProfileOpen && (
              <div className={`absolute right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-200 z-30 overflow-hidden ${isDtlEmployee ? 'w-[30rem]' : 'w-64'}`}>
                  ...
                  {isDtlEmployee ? (
                    <>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                         <span className="text-xs font-bold text-orange-500 capitalize">{user.role.replace(/_/g, ' ')}</span>
                         {user.role !== initialUser.role && <span className="text-[9px] bg-gray-100 px-1.5 rounded text-gray-500">(Simuleret)</span>}
                      </div>
                    </>
                  ) : (<><p className="text-xs text-gray-500">{user.clubName ?? 'Ukendt Klub'}</p><p className="text-xs text-gray-500">{user.clubFunction ?? user.role}</p></>)}
                </div>
                <div className="max-h-[70vh] overflow-y-auto custom-scrollbar">
                  <div className="p-1.5"><a href="#" className="flex items-center w-full text-left p-2 rounded-md hover:bg-gray-100 text-xs cursor-pointer text-black font-medium"><Settings className="h-4 w-4 mr-2 text-gray-400" />{headerDict.settings ?? 'Account Settings'}</a></div>
                  <div className="my-1 border-t"></div>
                  {(isDtlEmployee) && (
                    <div className="bg-white">
                       <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 flex items-center"><Wrench className="w-3 h-3 mr-2 text-orange-500" /> <span className="text-[10px] uppercase font-bold text-black tracking-wider">Developer Tools</span></div>
                       <div className="p-3">
                              ...
                              <div key={idx}>
                                ...
                                <div className="grid grid-cols-3 gap-1">
                                  {group.levels.map((level) => (
                                    <button key={level} onClick={() => handleLevelChange(level as SubscriptionLevel)} className={`text-[10px] py-1 px-1 rounded border transition-all font-medium ${user.subscriptionLevel === level ? 'bg-orange-500 border-orange-500 text-white shadow-sm' : 'bg-white border-gray-200 text-black hover:border-black'}`}>{level}</button>
                                  ))}
                                </div>
                              </div>
                            ))}
                       ...
                       <div className="p-3">
                          <div className="text-[10px] font-bold text-black mb-2 uppercase tracking-wider flex items-center"><Shield className="w-3 h-3 mr-1" /> Roller</div>
                                ...
                                <div key={idx}>
                                   ...
                                   <div className="grid grid-cols-2 gap-1">
                                      {group.roles.map((role) => (
                                        <button key={role} onClick={() => handleRoleChange(role)} className={`text-[10px] py-1.5 px-2 rounded border transition-all text-left truncate ${user.role === role ? 'bg-orange-500 border-orange-500 text-white font-bold shadow-sm' : 'bg-white border-gray-200 text-black hover:border-black'}`} title={role.replace(/_/g, ' ')}>{role.replace(/_/g, ' ')}</button>
                                      ))}
                                   </div>
                                </div>
                             ))}
                  ...
                  <div className="p-1.5">
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-100">
                      <p className="text-[10px] text-gray-400 px-2 text-center">{headerDict.access_level_label ?? (lang === 'da' ? 'Adgangsniveau:' : 'Access Level:')} <span className="font-medium text-gray-600">{user.subscriptionLevel}</span></p>
                    </div>
                  </div>
                </div>
  ...
  return (
    <div id="secure-layout-client-root" className="flex flex-col h-full bg-gray-50 text-black overflow-hidden">
      <Header />
      <div className="flex flex-1 overflow-hidden">
        <Sidebar />
        {isSidebarOpen && (<div className="fixed inset-0 bg-black/30 z-20 lg:hidden" onClick={() => setIsSidebarOpen(false)}></div>)}
