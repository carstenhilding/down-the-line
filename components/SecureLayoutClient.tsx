...
import { LayoutDashboard, Users, Zap, Menu, LogOut, Settings, User as UserIcon, Globe, ChevronDown, BookOpen, Calendar, Activity, Shield } from 'lucide-react';
...
const modules = [
    ...
    { nameKey: 'video', path: '/analysis', icon: Zap, role: 'developer' },
...
export default function SecureLayoutClient({ children, user, dict, lang, initialPathname }: SecureLayoutClientProps) {
  ...
  const accessibleModules = modules.filter(m => user.role === UserRole.Developer || (user.role === UserRole.Tester && m.role !== UserRole.Developer));
    ...
    if (language && newLang !== language) {
      ...
      setLanguage(newLang);
        ...
        console.warn("Language context not ready or lang unchanged.");
         const newPath = pathname.includes('/da/') ? pathname.replace('/da/', `/${newLang}`)
                     : pathname.includes('/en/') ? pathname.replace('/en/', `/${newLang}`)
                     : `/${newLang}${pathname}`;
         router.push(newPath);
         setIsProfileOpen(false);
         setLanguage(newLang);
           ...
           if (!module.subModules) {
             ...
             return (
               <Link key={module.nameKey} href={`/${lang}${module.path}`} className={`relative flex items-center p-2 rounded-lg transition-colors group cursor-pointer ${!isSidebarOpen && 'justify-center'} ${isActive ? 'bg-black text-orange-500 font-bold text-sm' : 'text-black hover:text-orange-500 text-xs'}`}>
                 ...
                 <span className={`ml-3 whitespace-nowrap transition-opacity ${isSidebarOpen ? 'opacity-100' : 'opacity-0'} ${!isSidebarOpen && 'hidden'}`}>{sidebarDict[module.nameKey] ?? module.nameKey}</span>
               </Link>
             );
           }
  ...
  // === LAYOUT STRUKTUR ===
  return (
    <div id="secure-layout-client-root" className="flex flex-col h-screen bg-gray-50 text-black overflow-hidden">
        <Header />
